...
>
  ...
  <div
    class="relative z-10 text-center max-w-lg px-8 py-16 {background
      ? 'text-white'
      : ''}"
  >
    {#if logo}
        ...
        {#if socials.tiktok?.trim()}
          ...
          >
            <svg
              ...
              stroke-linecap="round"
              stroke-linejoin="round"
              ><path d="M9 12a4 4 0 1 0 4 4V4a5 5 0 0 0 5 5"></path></svg
            >
          </a>
        {/if}
        {#if socials.youtube?.trim()}
          <a
            href={socials.youtube.trim()}
            target="_blank"
            rel="noopener noreferrer"
            class="opacity-60 hover:opacity-100 transition-opacity"
            aria-label="YouTube"
