<script>
  ...
  function handleSave() {
    ...
    if (isNew) {
      ...
      onUpdate(community.map(p => p.id === editing.id ? post : p));
    }
          ...
          <tr class="border-b border-border/50 hover:bg-muted/50 transition-colors">
            ...
            <td class="p-3 text-right">
              <div class="flex items-center justify-end gap-1">
                ...
                <button aria-label="Delete post" onclick={() => handleDelete(post.id)} class="p-1.5 text-muted-foreground hover:text-destructive transition-colors">
